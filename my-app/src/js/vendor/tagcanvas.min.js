...
(function () {
	...
	function t(aT, aL, aR) {
		var aS = parseInt(aT.toString().length * aR, 10), aK = parseInt(aR * 2 * aT.length,10), aI = P(aS, aK), aO, j, aJ, aN,
			aQ, aP, aH, aM;
		if (!aI) {
			return null
		}
	...
	function S(aJ, aK) {
		var aI, aH, i = parseInt(Y(aK, "width"),10) / aK.width, j = parseInt(Y(aK, "height"),10) / aK.height;
		if (ai(aJ.offsetX)) {
	...
	c.Clicked = function (aK) {
		...
		if (aH !== "" && aH !== "_self") {
			if (window.self.frames[aH]) {
				window.self.frames[aH].document.location = aI
			} else {
				try {
					if (window.top.frames[aH]) {
						window.top.frames[aH].document.location = aI;
						return
					}
				} catch (aJ) {
				}
